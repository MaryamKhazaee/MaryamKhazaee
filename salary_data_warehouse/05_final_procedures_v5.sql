...
-- =============================================
-- PART 2: ETL STORED PROCEDURES (Fixed v5 - Explicit Column Mapping)
-- =============================================

-- Dummy table for compilation (Generic, since we rely on runtime creation now)
IF OBJECT_ID('dbo.stg_SalaryRaw2', 'U') IS NOT NULL DROP TABLE dbo.stg_SalaryRaw2;
CREATE TABLE dbo.stg_SalaryRaw2 (DummyCol INT); 
GO
...
BEGIN
    ...
    -- A. Extract SnapMarket (Creates the table structure)
    ...
    -- B. Extract Zooket (Appends data)
    PRINT 'Fetching Zooket data (Year 04 only)...';
    
    -- FIX: Since columns are identical but "SELECT *" is failing with Msg 213,
    -- it means the ORDER of columns might be different, or there is a hidden system column mismatch.
    -- We will use a dynamically generated column list to ensure we insert explicitly by name.
    
    DECLARE @cols NVARCHAR(MAX);
    
    -- Get column names from the newly created table (excluding SourceSystem which we add manually)
    SELECT @cols = STRING_AGG(QUOTENAME(name), ', ') WITHIN GROUP (ORDER BY column_id)
    FROM sys.columns 
    WHERE object_id = OBJECT_ID('dbo.stg_SalaryRaw2')
      AND name != 'SourceSystem';

    DECLARE @sql NVARCHAR(MAX);
    SET @sql = N'
        INSERT INTO dbo.stg_SalaryRaw2 (' + @cols + N', SourceSystem)
        SELECT ' + @cols + N', ''Zooket''
        FROM OPENQUERY(REMOTE_VIEW3, ''SELECT * FROM View3.dbo.SalaryZooket WHERE PeriodName LIKE ''''% 04'''''')
    ';
    
    PRINT 'Executing dynamic insert for Zooket...';
